...
field_names = {
    "all": [
        'title', 'category', 'publisher', 'publish_date', 'item_name',
        'procurement_number', 'contract_name', 'contract_number', 'deal_price',
        'purchaser', 'supplier', 'contract_signed_date', 'appendix',
        'source_url', 'id'
    ],
    "sub":
    ['title', 'category', 'publisher', 'publish_date', 'source_url', 'id']
}
...
def get_download_link(origin_url, linkEl):
    try:
        ...
        if p.findall(fileLink):
            ...
            match = re.search(r'(.\/)*([(\.\.\/)]*)([\w0-9\/.*?=&]+)',
                              fileLink)
            if match:
                if not match.group(1) and not re.findall(match.group(2), '..'):
                    return [
                        get_domain(origin_url) + '/' + match.group(3), fileName
                    ]
                else:
...
def get_links(url, page_num=1):
    ...
    try:
        ...
        for container in links_container:
            ...
            link = {"title": link_title, "date": link_date, "url": abs_url}
        ...
        if error_count > 6:
            print(
                "An exception occurred, We will reprocess this action 1 hour later!",
                e)
            sleep(3600)
        else:
            print("Request again!")
...
def get_contract(url, category=''):
    ...
    try:
        ...
        try:
            ...
            data_container = page_html.select_one(
                "tbody div[style*='line-height']")
            ...
            if elLens > 0:
                contract.publisher = get_txt(
                    page_html.select("p[align='right']")[0])
            if elLens > 1:
                contract.publish_date = get_txt(
                    page_html.select("p[align='right']")[1])

            contract.appendix = get_txt(page_html.select_one('a[href*=down]'))
            all_links = [
                get_download_link(url, link)
                for link in page_html.select('a[href*=down]')
                if get_download_link(url, link)
            ]

            if category == 'contract_notices':
                contract.item_name = get_search_str(data_container, '项目名称')
                contract.procurement_number = get_search_str(
                    data_container, '项目编号')
                contract.contract_name = get_search_str(data_container, '合同名称')
                contract.contract_number = get_search_str(
                    data_container, '合同编号')
                contract.deal_price = get_search_str(data_container,
                                                     '合同金额\(万元\)')
                contract.purchaser = get_search_str(data_container, '采购单位')
                contract.supplier = get_search_str(data_container, '中标供应商')
                contract.contract_signed_date = get_search_str(
                    data_container, '合同签订日期')
...
def main():
    ...
    try:
        for target in base_urls:
            ...
            csv_file_path = 'results/' + CSV_PATH + target[
                "level"] + "/" + csv_file_name
            folder_prefix = 'results/' + FILE_PATH + target[
                "level"] + "/" + target["category"]
            processingId = target["level"] + "->" + target["category"]

            # Get log content to check what is a last action
            global log_status
            logIndex = next((i for i, item in enumerate(log_content)
                             if item["category"] == target["category"]
                             and item["level"] == target["level"]), -1)
            ...
            if len(new_links) > 0:
                ...
                add_console("Starting to grab contract detail for " +
                            processingId)
            else:
            ...
            for link in new_links:
                ...
                if contract:
                    if target["category"] == "contract_notices":
                        utils.download_html(folder_path, link.url,
                                            contract["page_body"], notice_id)
                        utils.download_files(folder_path,
                                             contract["file_links"], notice_id)
                        utils.append_new_row(csv_file_path,
